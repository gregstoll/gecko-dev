...
struct MaybeWebGLVariantMatcher {
  ...
  JS::Value match(const Int32Array2& x) {
    int32_t arr[] = { x[0], x[1] };
    JSObject* obj = dom::Int32Array::Create(mCx, mCxt, 2, arr);
    if (!obj) {
  ...
  JS::Value match(const Int32Array4& x) {
    int32_t arr[] = { x[0], x[1], x[2], x[3] };
    JSObject* obj = dom::Int32Array::Create(mCx, mCxt, 4, arr);
    if (!obj) {
      *mRv = NS_ERROR_OUT_OF_MEMORY;
      mCxt->EnqueueErrorOutOfMemory("ToJSValue: Out of memory.");
    }
  ...
  JS::Value match(const Uint32Array4& x) {
    uint32_t arr[] = { x[0], x[1], x[2], x[3] };
    JSObject* obj = dom::Uint32Array::Create(mCx, mCxt, 4, arr);
    if (!obj) {
      *mRv = NS_ERROR_OUT_OF_MEMORY;
      mCxt->EnqueueErrorOutOfMemory("ToJSValue: Out of memory.");
    }
  ...
  JS::Value match(const Float32Array2& x) {
    float arr[] = { x[0], x[1] };
    JSObject* obj = dom::Float32Array::Create(mCx, mCxt, 2, arr);
    if (!obj) {
      *mRv = NS_ERROR_OUT_OF_MEMORY;
      mCxt->EnqueueErrorOutOfMemory("ToJSValue: Out of memory.");
    }
  ...
  JS::Value match(const Float32Array4& x) {
    float arr[] = { x[0], x[1], x[2], x[3] };
    JSObject* obj = dom::Float32Array::Create(mCx, mCxt, 4, arr);
    if (!obj) {
      *mRv = NS_ERROR_OUT_OF_MEMORY;
      mCxt->EnqueueErrorOutOfMemory("ToJSValue: Out of memory.");
    }
  ...
  JS::Value match(const BoolArray4& x) {
    bool arr[] = { x[0], x[1], x[2], x[3] };
    JS::Rooted<JS::Value> obj(mCx);
    if (!dom::ToJSValue(mCx, arr, &obj)) {
      *mRv = NS_ERROR_OUT_OF_MEMORY;
      mCxt->EnqueueErrorOutOfMemory("ToJSValue: Out of memory.");
    }
    return obj;
  ...
  JS::Value match(const WebGLId<WebGLClass>& x) {
    return mCxt->WebGLObjectAsJSValue(mCx, MakeWebGLObject(x, RefPtr<ClientWebGLContext>(mCxt)), *mRv);
  }
