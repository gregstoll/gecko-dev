...
already_AddRefed<OpenSettingsPromise::Private> OpenWindowsLocationSettings() {
  ...
  HSTRING uriHstr = nullptr;
  ::WindowsCreateString(L"ms-settings:privacy-location", (DWORD)(wcslen(L"ms-settings:privacy-location")), &uriHstr);
  NS_ENSURE_TRUE(uriHstr, nullptr);
  RefPtr<IUriRuntimeClass> uri;
  HRESULT hr = uriFactory->CreateUri(uriHstr, getter_AddRefs(uri));
  ::WindowsDeleteString(uriHstr);
  if (FAILED(hr)) {
    return nullptr;
  }
...
bool SystemWillPromptForPermissionHint() {
  ...
  HSTRING wifiControlHstr = nullptr;
  ::WindowsCreateString(L"wifiControl", (DWORD)(wcslen(L"wifiControl")), &wifiControlHstr);
  NS_ENSURE_TRUE(wifiControlHstr, false);
  RefPtr<IAppCapability> appCapability;
  hr = appCapabilityStatics->CreateWithProcessIdForUser(
      user, wifiControlHstr, ::GetCurrentProcessId(), getter_AddRefs(appCapability));
  ::WindowsDeleteString(wifiControlHstr);
  if (FAILED(hr)) {
    return false;
  }
    ...
    [promise = RefPtr{aSystemPermissionPromise}, locationListener](bool aWasOpened){
      ...
      appCapability->add_AccessChanged(Callback<AccessChangedHandler>(
        [promise, locationListener](IAppCapability*, IAppCapabilityAccessChangedEventArgs*) {
          // Check that the location access changed to allow.  If so, resolve
          // the promise.
